...
#                configuration level dataset. If there is more than one fuel type, use fuel level data
# 
# Inputs:   
#         
#
...
emissions_by_unit <- fuel_level_data_w_naics |> 
  ...
  mutate(
    ghg_gas_name = case_when(
      ...
      ghg_gas_name == "tier123_biogenic_co2_emissions" ~ "Co2 Biogenic (unit level from configuration data)"
    )
  )|>
  filter(ghg_gas_name!="Co2 Biogenic (unit level from configuration data)")|>
